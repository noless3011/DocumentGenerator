        ...
        if item.endswith(f"_{project_id}"):
            ...
            if os.path.exists(metadata_path):
                ...
                if project:
                    ...
                    # Check if preview app HTML already exists
                    preview_app_path = os.path.join(project.output_dir, "preview_app.html")
                    preview_app_exists = os.path.exists(preview_app_path)
                    ...
                    # Include preview app data if it exists
                    if preview_app_exists:
                        try:
                            with open(preview_app_path, "r", encoding="utf-8") as f:
                                html_content = f.read()
                            
                            response_data["preview_app"] = {
                                "exists": True,
                                "path": preview_app_path,
                                "html": html_content
                            }
                        except Exception as e:
                            print(f"Error reading existing preview app: {str(e)}")
